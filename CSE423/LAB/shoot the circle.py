...
circle_color = (1.0, 1.0, 0.0)  # Red color for the circle
game_state = 'playing'
quit_flag = False
falling_circle = []
projectiles = []
...
misses = 0
missed_shots = 0
timer_active = False  # Added to control the timer

def draw_projectiles():
    glColor3f(1.0, 1.0, 0.0)  # Yellow color for projectiles
    for x, y, radius in projectiles:
        draw_circle(x, y, radius)

for i in range(6):
    x = random.randint(25, WIDTH - 25)  # random x points
    y = random.randint(650, 750)  # random y points
    radius = random.randint(15, 25)  # different sizes for raindrops
...
def keyboard(key, x, y):
    ...
    if key == b'\x1b':  # Escape key
        ...
        if shooter_pos - radius < 0:  # Prevent circle from going off-screen to the left
            shooter_pos = radius
    elif key == b'd':
        shooter_pos += step_size
        if shooter_pos + radius > WIDTH:  # Prevent circle from going off-screen to the right
            shooter_pos = WIDTH - radius
    elif key == b' ':
        projectiles.append((shooter_pos, 50, 10))
...
def update(value):
    ...
    if game_state == 'playing':
        ...
        for i in range(len(falling_circle)):
            if game_state!="over":
                ...
                if check_collision(x, y, radius, shooter_pos, 50, 20):  # Shooter position (shooter_pos, 50), shooter radius 20
                    game_state = 'over'
                    falling_circle.clear()
                    glutPostRedisplay()
    ...
    if game_state != 'over':
        ...
        timer_active = False  # Mark timer as inactive when the game is over

def check_collision(x1, y1, r1, x2, y2, r2):
    # Calculate the distance between the two centers
    distance = ((x2 - x1)**2 + (y2 - y1)**2)**0.5
    return distance < (r1 + r2)  # Check if the distance is less than the sum of the radii
